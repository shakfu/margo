<script lang="ts">
  ...
  // Knowledge sources — workspace-scoped RAG (TODO §6.6.A).
  const {
    elements: { root: kbRoot, trigger: kbTrig, content: kbContent },
    states: { open: kbOpen }
  } = mkSection(false);
  ...
  // Reload when this panel mounts and whenever the active workspace changes,
  // since the indexed-source list is per-workspace.
  onMount(refreshKbSources);
  $: if (mode === 'workspace') { void $settings.activeWorkspaceId; refreshKbSources(); }
...
<div class="flex flex-col h-full bg-bg-elev border-l border-border overflow-y-auto" use:melt={$tabsRoot}>
  ...
  {#if mode === 'workspace'}
  <!-- Knowledge sources — per-workspace RAG index (§6.6.A). Lives in
       the right pane because each workspace has its own collection. -->
