...
## 4. Agent run cancellation race

`CancelStream(id)` cancels the underlying context for an in-flight
`StreamAgent` run, but Eino's ReAct agent does not always surface the cancel
promptly — the loop typically finishes the current tool call (and sometimes
the current model turn) before the goroutine in `app.go:StreamAgent` returns.
For the trivial `current_time` tool this is unnoticeable; with slow tools
(network calls, LLM-as-tool) the user can wait several seconds after pressing
cancel. Fix options:

- Wrap each `tool.InvokableTool` we register in `builtinTools` with a
  ctx-aware decorator that returns early on `ctx.Done()`.
- Investigate Eino's `compose.WithRuntimeMaxSteps` / interrupt machinery for
  a graph-level cancel signal that's checked between nodes.

Either way, surface a "cancelling…" badge in `App.svelte` between the cancel
click and the `:done`/`:error` event so the user knows the request was
received.
...
### 6.1 Mid-loop text streaming (supersedes #5)

Smallest payoff-per-effort item. Use Eino's
`utils/callbacks.ModelCallbackHandler{OnEndWithStreamOutput}` to forward
each model turn's text deltas as `StepText` events. See task #5 for the
detailed plan; do this first because it's bounded and visibly improves
the agent UX.

### 6.2 Cancellation via interrupt (supersedes #4)

Investigate Eino's checkpoint/interrupt machinery (`compose` package's
`WithRuntimeMaxSteps`, interrupt-on-ctx hooks). The proper fix isn't
ctx-aware tool wrappers but a graph-level cancel signal Eino itself
honours between nodes. Surface a "cancelling…" badge in `App.svelte`.
